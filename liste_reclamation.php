...
$res = $bdd->query("select * from reclamation");
?>
<!DOCTYPE html>
<html>
		...
		<div class="menu-block customscroll" style="margin-top: 50px;">
			<div class="sidebar-menu">
				<ul id="accordion-menu">
					...
					<li>
						<a href="liste_reclamation.php" class="dropdown-toggle no-arrow">
					...
					<div class="pb-20" style="padding-top: 20px;">
						<table class="data-table table stripe hover nowrap">
							<thead>
								...
								<tr>
									<th>Enseignant</th>
									<th class="table-plus datatable-nosort">Nummero de salle</th>
							...
							<tbody>
								<?php while ($ligne = $res->fetch()) {
									$id = $ligne['id_ens'];
									$rs = $bdd->query("select * from enseignant where id_ens = $id");
									$ligne2 = $rs->fetch();
									?>

									<tr>
										<td><?php echo $ligne2['nom_ens']," ",$ligne2['prenom_ens'] ?></td>
										<td class="table-plus"><?php echo $ligne['numsalle'] ?></td>
										<td><?php echo $ligne['type'] ?></td>
										<td><?php echo $ligne['description'] ?></td>
										<td><?php echo $ligne['perioritee'] ?></td>
										<td><?php
											if ($ligne['etat'] == "en cours de traitement") {
												echo "<span class='badge badge-warning'>Non terminée</span>";
											} else if($ligne['etat'] == "terminee")
												echo "<span class='badge badge-success'>Terminée</span>";
										...
										<td>
											...
											<div class="dropdown-menu dropdown-menu-right dropdown-menu-icon-list">
												<a class="dropdown-item" href="controller/terminerReclamation.php?id=<?php  echo $ligne['id_reclamation'] ?>"><i class="dw dw-tick"></i> Terminer</a>
												<a class="dropdown-item" href="controller/supprReclamation.php?id=<?php  echo $ligne['id_reclamation'] ?>"><i class="dw dw-delete-3"></i> Supprimer</a>
											</div>
										</div>
										</td>
									</tr>
